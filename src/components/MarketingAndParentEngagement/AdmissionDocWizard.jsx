...
import { Upload, FileDown, ExternalLink, Check, Eye } from "lucide-react";
...
const AdmissionDocWizard = () => {
  ...
  // Step 4 — newsletter
  const [newsletterTemplate, setNewsletterTemplate] = useState(newsletterTemplates[0]);
  ...
  const totalEvents = monthWorksheets.length;
  ...
  const buildPdfDoc = () => {
    ...
    drawTable(
      ...
      assessments.map((a) => [
        a.subjectname,
        a.pdfLink ? { text: "View PDF", link: a.pdfLink } : "Coming soon",
      ]),
      [125, 55]
    );
  ...
  const handleDownloadPdf = () => {
    ...
    doc.save(`${schoolName || "admission"}-${classLevel}-${month}.pdf`);
  };
